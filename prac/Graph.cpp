...
//내가 추가한 함수
void Graph::AddName(int vertexKey,string name) {//이름을 추가하는 함수
    Vertex* cur = FindVertex(vertexKey);
    if (cur == NULL) { return; }
    else { cur->SetName(name); }
}
void Graph::Addstore(int vertexKey, string store) {//이름을 추가하는 함수
    Vertex* cur = FindVertex(vertexKey);
    if (cur == NULL) {return; }
...
// 이미 있는 key값을 삽입할 때는 구현 따로 안함(해야하면 그 노드 지우고 채우는걸로하기)
void Graph::AddVertex(int vertexKey) {
    Vertex* cur = m_pVHead;
    Vertex* inv = new Vertex(vertexKey);//이렇게 해도 초기화 되나? ine는 삽입할 vertex
