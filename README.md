...
pivot을 기준으로 pivot보다 작다면 pivot의 왼쪽으로, pivot보다 크다면 pivot의 오른쪽으로 옮겨진다. 옮겨지고 난 뒤 pivot을 제외하고 왼쪽과 오른쪽을 다시 정렬하는데 pivot을 기준으로 나눠진 두개의 배열에 새로운 pivot을 정해서 계속해서 반복한다(재귀함수). 계속해서 나눠지다가 배열의 크기가 0이거나 1이 된다면 종료한다.
