...
5. FLOYD
다른 알고리즘들과는 달리 모든 정점 쌍에 대해 최단 거리를 구하고 음의 비용을 갖는 간선이 있어도 사용할 수 있다.
플로이드 알고리즘은 s에서 e까지 이동할 때 최단거리를 구한다. 이때 중간 경유지인 w를 이용한다. 즉 s에서 w까지의 최단거리와 w에서 e까지의 최단거리를 이용한다.
