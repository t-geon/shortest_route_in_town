...
Heap sort는 원래 방식인 정형적인 방식으로 구현하지 않고 프로젝트를 진행할 때 구현해야 했던 min heap으로 구현했다. 즉 push를 이용해 heap에 값을 넣어준다. push를 할 때는 맨 끝에 삽입한 후 자신의 부모(자신 인데스/2)와 비교해 작으면 위로 부모와 위치를 바꾼다. 최대 root까지 올라갈 수 있다.
5 3 7 1 을 정렬한다면 
5는 root가 된다.
